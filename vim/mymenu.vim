...
nnoreme 10.40 文件(&F).保存副本(&Y) :browse w<CR>
...
nnoreme 20.22 编辑(&E).显示字符的UTF-8编码(&8)<Tab>g8 g8
nnoreme 20.44 编辑(&E).插入UTF-8编码的C转义序列(&U) :call EnterEncodedEscapeSequences("utf-8")<CR>
nnoreme 20.45 编辑(&E).插入GBK编码的C转义序列(&K) :call EnterEncodedEscapeSequences("gbk")<CR>
...
nnoreme 40.52 工具(&T).启用拼写检查(&K) :setlocal spell spelllang=en,cjk<CR>
