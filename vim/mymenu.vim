...
noreme 10.50 文件(&F).&Netrw <Nop>
noreme 10.9990 文件(&F).-2- <Nop>
noreme 10.9999 文件(&F).退出(&X) :qa<CR>
noreme 20.10 编辑(&E).十六进制编辑(&C) :call XxdConv()<CR>
noreme 20.11 编辑(&E).十六进制编辑返回(&R) :call XxdBack()<CR>
noreme 20.20.10 编辑(&E).大幅改动(&B).删除空格(&D) :%s/ //g<CR>
noreme 20.20.15 编辑(&E).大幅改动(&B).删除空白(&W) :%s/\s+//g<CR>
noreme 20.20.20 编辑(&E).大幅改动(&B).奇偶行分离(&E) :call SeparateEvenOddLines()<CR>
